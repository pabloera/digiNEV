...
class AdaptiveChunkingManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.default_chunk_size = self.config.get('chunk_management', {}).get('base_chunk_size', 10)
        self.max_chunk_size = self.config.get('chunk_management', {}).get('max_chunk_size', 50)
        self.min_chunk_size = self.config.get('chunk_management', {}).get('min_chunk_size', 2)
    ...
    def _load_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"⚠️ Arquivo de configuração não encontrado: {config_path}")
    ...
    def get_optimal_chunk_size(self, stage_name: str, data_size: int, 
        ...
        stage_specific = self.config.get('chunk_management', {}).get('stage_specific_chunks', {})
        if stage_name in stage_specific:
            configured_size = stage_specific[stage_name]
            self.logger.info(f"📋 Usando chunk size configurado para {stage_name}: {configured_size}")
            return max(self.min_chunk_size, min(configured_size, self.max_chunk_size))
        ...
        if stage_name in self.optimal_chunk_sizes:
            ...
            self.logger.info(f"📊 Usando chunk size histórico para {stage_name}: {historical_optimal}")
            return historical_optimal
            
        # 3. Calcular baseado no tempo target
        target_time = self.config.get('chunk_management', {}).get('time_per_chunk_target', 60)
        calculated_size = int(target_time / estimated_time_per_record)
        
        # 4. Aplicar limitações
        optimal_size = max(self.min_chunk_size, min(calculated_size, self.max_chunk_size))
        
        # 5. Ajustar para stages API-intensivos
        api_intensive_stages = ['05_political_analysis', '08_sentiment_analysis', 
        ...
        if stage_name in api_intensive_stages:
            optimal_size = max(2, int(optimal_size * 0.5))  # Reduzir pela metade para API
            self.logger.info(f"⚡ Reduzindo chunk size para stage API-intensivo {stage_name}: {optimal_size}")
            
        self.logger.info(f"✅ Chunk size ótimo calculado para {stage_name}: {optimal_size}")
        return optimal_size
        
    def create_adaptive_chunks(self, data: pd.DataFrame, stage_name: str, 
                              estimated_time_per_record: float = 2.5) -> List[pd.DataFrame]:
        """
        ...
        """
        data_size = len(data)
        optimal_chunk_size = self.get_optimal_chunk_size(stage_name, data_size, estimated_time_per_record)
        
        self.logger.info(f"📦 Criando chunks adaptativos para {stage_name}")
        self.logger.info(f"📊 Dataset: {data_size} registros | Chunk size: {optimal_chunk_size}")
    ...
    def process_with_adaptive_chunking(self, data: pd.DataFrame, stage_name: str,
...
def with_adaptive_chunking(stage_name: str, estimated_time_per_record: float = 2.5):
    """
    Decorator para aplicar chunking adaptativo automaticamente a funções de processamento
    
    Usage:
        @with_adaptive_chunking("08_sentiment_analysis", estimated_time_per_record=3.0)
