...
class BatchConfig:
    ...
    def load_political_lexicon(cls):
        """Load political lexicon from JSON file - REQUIRED"""
        ...
        if not lexicon_path.exists():
            raise FileNotFoundError(
                f"ERRO CRÍTICO: Arquivo de léxico não encontrado: {lexicon_path}\n"
                f"O arquivo 'lexico_politico_hierarquizado.json' é obrigatório para o funcionamento do sistema."
            )
        ...
        try:
            ...
            raise ValueError(f"Erro ao decodificar JSON do léxico: {e}")
        except Exception as e:
            raise RuntimeError(f"Erro ao carregar léxico político: {e}")
...
class IntegratedBatchAnalyzer:
    ...
    def _load_lexicon(self):
        """Load political lexicon from JSON file - REQUIRED, no fallback"""
        # Load from JSON file (will raise exception if not found)
        ...
        logger.info(f"   - {len(self.political_keywords)} categorias políticas")
        logger.info(f"   - {len(self.transversal_keywords)} macrotemas")
