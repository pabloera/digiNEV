...
class PoliticalAnalyzer(AnthropicBase):
    ...
    def _smart_filter_political_relevance(self, df: pd.DataFrame, text_column: str) -> pd.DataFrame:
        """
        OPTIMIZED FILTERING for Brazilian political discourse analysis

        RESEARCH-BASED IMPROVEMENTS:
        - Balanced precision/recall for better coverage of political content
        - Tiered filtering approach (essential vs. quality filters)
        - Brazilian political context awareness
        - Reduced false negatives while maintaining quality
        """

        logger.info(f"🎯 Starting political relevance filtering for {len(df)} records")

        # ESSENTIAL FILTERS (must pass to be considered)
        essential_conditions = [
            df[text_column].notna(),  # Must have content
            df[text_column].str.len() >= 5,  # Minimum viable length (reduced from 20)
        ]

        # QUALITY FILTERS (weighted scoring approach)
        quality_filters = {
            'not_mega_duplicate': df.get('duplicate_frequency', pd.Series([1] * len(df), index=df.index)) <= 200,  # Relaxed from 100
            'reasonable_length': df.get('text_length', pd.Series([10] * len(df), index=df.index)) >= 10,  # Reduced from 20
            'not_very_short': ~df.get('is_very_short', pd.Series([False] * len(df), index=df.index)),
        }

        # ENHANCED POLITICAL CONTENT DETECTION
        # Core political keywords (high precision)
        core_political_keywords = [
            'bolsonaro', 'lula', 'presidente', 'governo', 'política', 'eleição',
            'direita', 'esquerda', 'pt', 'psl', 'pl', 'psol', 'urna', 'voto',
            'congresso', 'stf', 'supremo', 'mito', 'capitão'
        ]

        # Extended political keywords (broader coverage)
        extended_political_keywords = [
            'militar', 'patriota', 'brasil', 'comunista', 'fascista',
            'golpe', 'ditadura', 'democracia', 'impeachment', 'corrupção',
            'deputado', 'senador', 'ministro', 'tse', 'alexandre', 'moraes',
            'fake news', 'mídia', 'globo', 'antagonista'
        ]

        # Political entities and figures
        political_entities = [
            'temer', 'dilma', 'fhc', 'sergio moro', 'glenn greenwald',
            'datafolha', 'ibope', 'globo', 'jovem pan', 'sbt', 'record'
        ]

        text_lower = df[text_column].fillna('').str.lower()

        # Multi-tier political content detection
        core_political = text_lower.apply(
            lambda x: bool(self._political_pattern.search(x)) if pd.notna(x) else False
        )

        # Extended political pattern (for broader coverage)
        extended_pattern = '|'.join(extended_political_keywords + political_entities)
        extended_political = text_lower.str.contains(extended_pattern, case=False, na=False)

        # Political context indicators (hashtags, mentions, etc.)
        context_indicators = text_lower.str.contains(
            r'#(bolsonaro|lula|eleição|política|brasil|pt|psl)|@(jairbolsonaro|lulaoficial)',
            case=False, na=False, regex=True
        )

        # TIERED FILTERING APPROACH
        # Tier 1: High confidence political content (core keywords)
        tier1_political = core_political

        # Tier 2: Extended political content (broader keywords)
        tier2_political = extended_political & ~tier1_political

        # Tier 3: Contextual political content (mentions, hashtags)
        tier3_political = context_indicators & ~tier1_political & ~tier2_political

        # Apply essential filters first
        essential_mask = essential_conditions[0]
        for condition in essential_conditions[1:]:
            essential_mask &= condition

        logger.info(f"📊 Essential filters: {len(df)} → {essential_mask.sum()} records")

        # Apply tiered political filtering
        # Tier 1: Keep all that pass essential + core political
        tier1_mask = essential_mask & tier1_political

        # Tier 2: Keep extended political that also pass quality filters
        quality_score = sum(quality_filters.values())
        tier2_mask = essential_mask & tier2_political & (quality_score >= 2)  # At least 2/3 quality filters

        # Tier 3: Keep contextual political with high quality score
        tier3_mask = essential_mask & tier3_political & (quality_score >= 3)  # All quality filters

        # Combine all tiers
        final_mask = tier1_mask | tier2_mask | tier3_mask

        # Log filtering results
        logger.info(f"🎯 Political filtering results:")
        logger.info(f"   Tier 1 (core political): {tier1_mask.sum()} records")
        logger.info(f"   Tier 2 (extended political): {tier2_mask.sum()} records")
        logger.info(f"   Tier 3 (contextual political): {tier3_mask.sum()} records")
        logger.info(f"   Final result: {len(df)} → {final_mask.sum()} records ({(final_mask.sum()/len(df)*100):.1f}% retention)")

        filtered_df = df[final_mask].copy()

        # Emergency fallback: if we filter out everything, keep top political matches
        if len(filtered_df) == 0:
            logger.warning("⚠️ All records filtered out, applying emergency fallback")
            # Keep records with any political keyword + basic quality
            emergency_mask = essential_mask & (tier1_political | tier2_political | tier3_political)
            if emergency_mask.sum() > 0:
                filtered_df = df[emergency_mask].copy()
                logger.info(f"🚨 Emergency fallback: kept {len(filtered_df)} records")
            else:
                # Last resort: keep first 10% of records that pass essential filters
                emergency_df = df[essential_mask].copy()
                if len(emergency_df) > 0:
                    keep_count = max(1, len(emergency_df) // 10)
                    filtered_df = emergency_df.head(keep_count).copy()
                    logger.warning(f"🚨 Last resort: kept {len(filtered_df)} records for analysis")
        ...
        try:
            ...
            if lexicon_path.exists():
                with open(lexicon_path, 'r', encoding='utf-8') as f:
                    ...
                    # Transform the hierarchical structure to the expected flat format
                    if "lexico" in data:
                        transformed = {"brazilian_political_lexicon": {}}
                        for macrotema, macrotema_data in data["lexico"].items():
                            if isinstance(macrotema_data, dict) and "subtemas" in macrotema_data:
                                for subtema, subtema_data in macrotema_data["subtemas"].items():
                                    if isinstance(subtema_data, dict) and "palavras" in subtema_data:
                                        # Use subtema as category key
                                        transformed["brazilian_political_lexicon"][subtema] = subtema_data["palavras"]
                        return transformed
                    else:
